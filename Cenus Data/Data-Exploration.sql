...
WITH state_avg_growth AS (   SELECT state,ROUND(AVG(growth)*100,4) AS avg_growth
                FROM data1
                GROUP BY 1
            ),
    min_avg_growth AS ( SELECT MIN(avg_growth) FROM state_avg_growth),
    max_avg_growth AS (SELECT MAX(avg_growth) FROM state_avg_growth)
SELECT * FROM state_avg_growth
WHERE avg_growth IN  (
...
SELECT  d2.district, d2.state, d1.sex_ratio, d2.population, 
        ROUND( population/ ((   cast(d1.sex_ratio as decimal ) / 1000   ) + 1)) AS males,
		population - ROUND( population/ (( cast(d1.sex_ratio as decimal)/ 1000) + 1))  AS females
FROM data2 d2 
JOIN data1 d1 
...
SELECT state,district,literacy,ranks
FROM (
        SELECT state,district, literacy, 
                RANK() OVER (PARTITION BY state ORDER BY literacy DESC) as ranks
        FROM data1 
    ) as sub
WHERE ranks <=3;
